...
#define API_KEY ""//API_Key for firebase
#define DATABASE_URL ""  //<databaseName>.firebaseio.com or <databaseName>.<region>.firebasedatabase.app
#define USER_EMAIL ""//Firebase cred
#define USER_PASSWORD ""//Firebase pass
...
void loop() {
  ...
  if (Firebase.ready() && (millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0)) {
    ...
    if (Speed != NewSpeed) { WriteFanVal(Speed); }
    //Firebase.RTDB.getString(&fbdo, F("/PowerOn")) ? fbdo.to<const char *>() : fbdo.errorReason().c_str();//Need to add PowerOnTheFan
